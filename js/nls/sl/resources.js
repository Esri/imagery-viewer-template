/*global define */
/*
 | Copyright 2018 Esri
 |
 | Licensed under the Apache License, Version 2.0 (the "License");
 | you may not use this file except in compliance with the License.
 | You may obtain a copy of the License at
 |
 |    http://www.apache.org/licenses/LICENSE-2.0
 |
 | Unless required by applicable law or agreed to in writing, software
 | distributed under the License is distributed on an "AS IS" BASIS,
 | WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 | See the License for the specific language governing permissions and
 | limitations under the License.
 */
define({
  "map": {
    "error": "Karte ni mogoče ustvariti",
    "licenseError": {
      "message": "Vaš račun ni licenciran za uporabo konfigurabilnih aplikacij, ki niso javne. Prosite administratorja v vaši organizaciji, da vam dodeli tip uporabnika, ki vsebuje aplikacije Essential Apps ali licenco za dodatne aplikacije Essential Apps.",
      "title": "Brez licence"
    }
  },
  "nav": {
    "close": "Zapri"
  },
  "basemap": {
    "title": "Galerija temeljnih kart"
  },
  "operationalLayers": {
    "title": "Operativni sloji",
    "error": "Na karti ni operativnih slojev."
  },
  "singleLayerViewer": {
    "title": "Izbirnik slikovnega sloja",
    "enable": "Poiščite posamezne slike",
    "tooltip": "Omogoči iskanje določenih slik.",
    "secondary": "Primerjalni sloj nastavi kot aktiven.",
    "dropDown": "Pokaži slike na spustnem seznamu.",
    "refresh": "Gumb Osveži",
    "refreshTooltip": "Osveži poizvedbo glede na trenutni obseg.",
    "renderer": "Upodabljanje",
    "layer": "Sloj",
    "show": "Prikaži",
    "age": "Iskalni doseg",
    "zoom": "Povečaj za izbiranje slik.",
    "error": "Na karti ni vidnih slikovnih slojev.",
    "error1": "Polje ni določeno.",
    "error2": "Ni polja OBJECTID.",
    "error3": "Ni polja kategorije.",
    "error4": "Dejanja za sloj ni mogoče izvajati.",
    "error5": "Storitve pred različico 10.2.1 niso podprte.",
    "error6": "V trenutnem obsegu ni 3D-prizorov.",
    "error7": "Število izbranih odtisov presega 20. Prikazanih bo samo prvih 20. Pritisnite V redu, da vas ne opozori znova.",
    "error8": "Podatki o sliki so na voljo samo med ",
    "error9": "Neveljaven razpon podatkov: datum začetka mora biti manj kot datum zaključka.",
    "slider": "Pokaži slike na drsniku.",
    "ageOption1": "Dnevi",
    "ageOption2": "Tedni",
    "ageOption3": "Meseci",
    "ageOption4": "Leta",
    "showOption1": "Slika",
    "showOption2": "Odtis",
    "date": "Datumi",
    "imageLabel": "slike",
    "default": "Privzeto"
  },
  "twoLayerViewer": {
    "title": "Izbirnik sloja",
    "enable": "Poiščite posamezne slike",
    "tooltip": "Omogoči iskanje določenih slik.",
    "secondary": "Primerjalni sloj nastavi kot aktiven.",
    "dropDown": "Pokaži slike na spustnem seznamu.",
    "refresh": "Gumb Osveži",
    "refreshTooltip": "Osveži poizvedbo glede na trenutni obseg.",
    "renderer": "Upodabljanje",
    "layer": "Sloj",
    "show": "Prikaži",
    "age": "Iskalni doseg",
    "zoom": "Povečaj za izbiranje slik.",
    "error": "Na karti ni vidnih slikovnih slojev.",
    "error1": "Polje ni določeno.",
    "error2": "Ni polja OBJECTID.",
    "error3": "Ni polja kategorije.",
    "error4": "Dejanja za sloj ni mogoče izvajati.",
    "error5": "Storitve pred različico 10.2.1 niso podprte.",
    "error6": "V trenutnem obsegu ni 3D-prizorov.",
    "error7": "Število izbranih odtisov presega 20. Prikazanih bo samo prvih 20. Pritisnite V redu, da vas ne opozori znova.",
    "error8": "Podatki o sliki so na voljo samo med ",
    "error9": "Neveljaven razpon podatkov: datum začetka mora biti manj kot datum zaključka.",
    "slider": "Pokaži slike na drsniku.",
    "ageOption1": "Dnevi",
    "ageOption2": "Tedni",
    "ageOption3": "Meseci",
    "ageOption4": "Leta",
    "showOption1": "Slika",
    "showOption2": "Odtis",
    "left": "Leva slika",
    "right": "Desna slika",
    "identicalLayerError": "Leva in desna slika sta identični.",
    "date": "Datumi",
    "imageLabel": "slike",
    "default": "Privzeto"
  },
  "editor": {
    "title": "Urednik",
    "error": "Možnost Uredi sloj ni bila najdena.",
    "error1": "Dostop zavrnjen. Slojev ni mogoče urejati.",
    "text": "Izberite simbol in kliknite na karti."
  },
  "measurement": {
    "title": "Meritve slikovja",
    "error": "Metrične zmožnosti niso podprte."
  },
  "export": {
    "title": "Izvoz",
    "mode": "Shrani lokacijo",
    "titleText": "Ime (zahtevano)",
    "description": "Opis",
    "tags": "Oznake (zahtevano)",
    "preview": "Predogled",
    "submit": "Shrani",
    "cancel": "Prekliči",
    "pixel": "Velikost pikslov",
    "outsr": "Izhodni koordinatni sistem",
    "renderer": "Možnosti prenosa TIFF",
    "formatText1": "Kot je prikazano",
    "formatText2": "Neobdelani podatki (vsi pasovi)",
    "extent": "Nariši poligon za določanje obsega",
    "drawText": "(kliknite sliko za začetek)",
    "text": "Neobdelanih podatkov ni mogoče prikazati s standardnimi pregledovalniki slik. Odpri z ArcGIS Pro.",
    "error": "Na karti ni vidnih slikovnih slojev.",
    "error1": "Zahtevano je ime.",
    "error2": "Zahtevane so oznake.",
    "error3": "PixelSize izvoza je omejen na",
    "error4": "za ta obseg.",
    "error5": "Vnesite veljavno številsko vrednost.",
    "error6": "Vaše slike trenutno ni mogoče izvoziti.",
    "thumbnailError": "Sličica ni na voljo",
    "advance": "Napredne možnosti shranjevanja",
    "modeOption1": "Shrani na portal",
    "modeOption2": "Shrani na disk",
    "default": "Privzeto",
    "utm": "WGS84 UTM cona",
    "layer": "Sloj",
    "mercator": "WebMercatorAS",
    "folder": "Izberi mapo"
  },
  "imageDate": {
    "label": "Datum slike"
  },
  "about": {
    "title": "Več o tem"
  },
  "bookmark": {
    "title": "Zaznamki",
    "selectBookmark": "Izberi zaznamke",
    "default": "Privzeto",
    "add": "Dodaj zaznamke",
    "addTitle": "Vnesite ime",
    "addBtn": "Dodaj začasno"
  },
  "coordinate": {
    "_widgetLabel": "Koordinata",
    "hintMessage": "Za pridobitev koordinat kliknite karto",
    "defaultLabel": "Privzeto",
    "realtimeLabel": "Za pridobitev koordinat premaknite miško",
    "computing": "Računam...",
    "latitudeLabel": "Geografska širina",
    "longitudeLabel": "Geografska dolžina",
    "loading": "nalaganje...",
    "enableClick": "Kliknite, da omogočite klik na zemljevid za pridobivanje koordinat",
    "disableClick": "Kliknite, da onemogočite klik na zemljevid za pridobivanje koordinat",
    "Default": "Privzeto",
    "Inches": "Palci",
    "Foot": "Čevlji",
    "Foot_US": "Čevlji_ZDA",
    "Yards": "Jardi",
    "Miles": "Milje",
    "Nautical_Miles": "Morske milje",
    "Millimeters": "Milimetri",
    "Centimeters": "Centimetri",
    "Meter": "Metri",
    "Kilometers": "Kilometri",
    "Decimeters": "Decimetri",
    "Decimal_Degrees": "Stopinje",
    "Degree_Minutes_Seconds": "Stopinje minute sekunde",
    "MGRS": "MGRS",
    "USNG": "USNG"
  }
});